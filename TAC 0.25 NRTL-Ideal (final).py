...
T_steam_1atm = 373.124 # Saturated steam temperature at 1 atm
...
latent_heat_steam_1atm = 2256.54 # latent heat for 1 atm saturated steam in kJ/kg
t = 8000 # annual operating time in h
water_cost = 0.00031701  # water cost in dollars/kg
steam_cost_11atm = 0.016314  # saturated steam (11 atm) cost in dollars/kg
steam_cost_1atm = 0.011023  # saturated steam (1 atm) cost in dollars/kg
payback_period = 3  # payback period in years

#### VARIABLES
roL_LPC = 778   # LPC liquid density in kg/m3
roV_LPC = 1.65  # LPC vapor density in kg/m3
roL_HPC = 709   # HPC liquid density in kg/m3
roV_HPC = 17.0  # HPC vapor density in kg/m3
V_LPC = 1.02871 # LPC vapor flowrate in m3/s
V_HPC = 0.11515 # HPC vapor flowrate in m3/s
N_trays_LPC = 36    # LPC number of trays
N_trays_HPC = 38    # HPC number of trays
Q_condenser_LPC = 3609900  # condenser duty for LPC in KJ/h
Q_reboiler_LPC = 3201600   # reboiler duty for LPC in KJ/h
Q_condenser_HPC = 0  # condenser duty for HPC in KJ/h
Q_reboiler_HPC = 3603700   # reboiler duty for HPC in KJ/h
T_dist_LPC = 333.015    # LPC distillate temperature in K
T_res_LPC = 342.985     # LPC residue temperature in K
T_dist_HPC = 410.099    # HPC distillate temperature in K
T_res_HPC = 434.304     # HPC residue temperature in K
...
DeltaT_kettle_LPC = T_steam_1atm - T_res_LPC
...
A_kettle_LPC = (Q_reboiler_LPC/3600)/(U_kettle*DeltaT_kettle_LPC)
...
column_height_HPC_ft = (Sm * (N_trays_HPC - 2) + 4.6) * ft_m
...
DeltaT_condenser_HPC = ((T_dist_HPC - T_water_in) - (T_dist_HPC - T_water_out)) / math.log((T_dist_HPC - T_water_in) / (T_dist_HPC - T_water_out))  # math.log() replace ln
A_kettle_HPC = (Q_reboiler_HPC/3600) / (U_kettle * DeltaT_kettle_HPC)
A_kettle_HPC_ft = A_kettle_HPC * (ft_m ** 2)
A_condenser_HPC = (Q_condenser_HPC/3600) / (U_condenser * DeltaT_condenser_HPC)
A_condenser_HPC_ft = A_condenser_HPC * (ft_m ** 2)
kettle_cost_HPC = (M_and_S / 280) * 101.3 * (A_kettle_HPC_ft ** 0.65) * (2.29 + Fc_kettle)
condenser_cost_HPC = (M_and_S / 280) * 101.3 * (A_condenser_HPC_ft ** 0.65) * (2.29 + Fc_condenser)
capital_cost_HPC = vessel_cost_HPC + trays_cost_HPC + kettle_cost_HPC + condenser_cost_HPC
...
# Capital cost HPC:
print("column_diameter_HPC = " + str(round(column_diameter_HPC,2)) + " m")  # str() to convert the numeric value to a string
print("column_height_HPC = " + str(column_height_HPC) + " m")               # round (parameter, number of decimals)
...
print("DeltaT_condenser_HPC = " + str(DeltaT_condenser_HPC) + " K")
print("A_kettle_HPC = " + str(A_kettle_HPC) + " m2")
print("A_condenser_HPC = " + str(A_condenser_HPC) + " m2")
print("kettle_cost_HPC = " + str(kettle_cost_HPC) + " $")
print("condenser_cost_HPC = " + str(condenser_cost_HPC) + " $")
